...
__global__ void moe_wna16_gemm_kernel(
  ...
  const uint32_t* expert_qzeros =
      has_zp ? qzeros + expert_offset / group_size / pack_factor : nullptr;
...
void run_moe_wna16_gemm(const scalar_t* input, scalar_t* output,
  ...
  if (bit == 4) {
    ...
    // }
    STD_TORCH_CHECK(false, "Unsupported bit width");
  }
...
torch::stable::Tensor moe_wna16_gemm(
  ...
  STD_TORCH_CHECK(bit == 4, "bit must be 4");
  ...
  STD_TORCH_CHECK(BLOCK_SIZE_M <= 64,
  ...
  if (input.scalar_type() == torch::headeronly::ScalarType::Half) {
    run_moe_wna16_gemm<half>(
        ...
        reinterpret_cast<const half*>(b_scales.const_data_ptr()),
    ...
    STD_TORCH_CHECK(false, "moe_wna16_gemm only supports float16 and float32");
  }
  return output;
}
